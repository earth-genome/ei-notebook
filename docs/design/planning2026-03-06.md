...
- **pos_ids / neg_ids:** Keep storing **positions** (results of `sindex.nearest`), as in the previous diagnosis.
- **Layer updates and save:** Use **`.iloc`** when indexing into `self.gdf` (e.g. `self.gdf.iloc[self.pos_ids][["geometry"]]`, and same for neg and for `save_dataset`).
